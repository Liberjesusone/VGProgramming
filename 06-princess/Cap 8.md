# Capítulo 8: Espadas en Movimiento y una Mazmorra Viva: Princess
...
> **[ ESPACIO PARA IMAGEN: Fotografía de Shigeru Miyamoto de medio cuerpo, vistiendo traje oscuro, camisa blanca y corbata ]**
> 
> 
> *Figura 8.1: Shigeru Miyamoto en 2015, codirector de The Legend of Zelda junto con Takashi Tezuka. Fotografía del Ministerio de Educación, Cultura, Deportes, Ciencia y Tecnología de Japón, licencia CC BY 4.0, vía Wikimedia Commons.*
> 
...
> **[ ESPACIO PARA IMAGEN: Diagrama de bloques que muestra dos estados apilados. El bloque superior dice "PauseState (activo)" y el inferior "PlayState (congelado)". Se indica que "render()" afecta a TODOS de abajo hacia arriba, mientras que "update() / on_input()" afecta SOLO al de la cima. ]**
> 
> 
...
| --- | --- | --- |
| **Cambiar de estado** | `change`: saca el estado anterior con `exit()` y lo sustituye por uno nuevo

 | `push`/`pop`: `push` agrega un estado encima sin tocar los demás; `pop` saca solo el de la cima con `exit()`<br> |
| **Quién recibe update/on_input** | Siempre uno solo: `self.current`<br> | Siempre uno solo: `self.states[-1]` (la cima)

 |
| **Quién se dibuja** | Solo el estado actual, `self.current`<br> | Todos los apilados, de abajo hacia arriba (`for state in self.states`)
...
* **Cambiar de estado** en `StateMachine` es `change`: saca el estado anterior (`exit`) y lo sustituye. En `StateStack` hay dos operaciones separadas, `push` (que agrega un estado encima de los que ya había, sin tocarlos, y lo entra con `enter`) y `pop` (que saca solamente el que está en la cima con `exit` y lo elimina de la lista), dejando expuesto de nuevo el que estaba justo debajo, intacto.


* **Quién recibe entrada y actualización:** en ambas clases, solo un estado la recibe, `self.current` en una, `self.states[-1]` (la cima) en la otra. En esto no hay diferencia: nunca hay dos estados compitiendo por el mismo evento de teclado ni actualizándose dos veces en el mismo cuadro.


* **Quién se dibuja:** aquí está la diferencia real. `StateMachine.render` dibuja únicamente `self.current`, porque solo existe un estado. `StateStack.render` recorre toda la lista, de abajo hacia arriba (`for state in self.states`), así que un estado que dejó de recibir `update` por no estar en la cima sigue dibujándose cuadro a cuadro, exactamente como quedó congelado la última vez que se actualizó.
...
def _try_interact(self) -> None:
    ...
    for npc in self.current_region().npcs:
        # Lógica de distancia dx, dy omitida
        if dx <= 1 and dy <= 1:
...
> **[ ESPACIO PARA IMAGEN: Diagrama de secuencia (UML) mostrando la interacción entre Jugador, EstadoDeMundo, StateStack y DialogueState. El jugador inicia la interacción (on_input), el EstadoDeMundo hace push del DialogueState en el StateStack, lo que llama a enter(). Luego, el jugador confirma (on_input), y StateStack hace pop() llamando a exit() en DialogueState. ]**
> 
> 
> *Figura 8.3: Ciclo completo de push/pop sobre StateStack para el cuadro de diálogo de Ultimate Fantasy: arriba, el push que apila DialogueState y ejecuta su enter(); abajo, cuadros después, el pop que ejecuta su exit() y lo retira de la pila. EstadoDeMundo recibe la llamada inicial que dispara el push pero ninguna otra después, ni exit() ni update(), así que su ausencia de la mitad inferior del diagrama es, precisamente, la evidencia de que quedó congelado, tal cual estaba, sin haber sido tocado.*
> 
...
> **[ ESPACIO PARA IMAGEN: Diagrama de máquina de estados con dos nodos: 'idle' y 'walk'. Muestra una transición de idle a walk (con la condición "wait_timer > wait_duration aleatorio, 1-5 s") y una transición de walk a idle (con la condición "movement_timer > move_duration y random.randint(1,3) == 1"). ]**
> 
> 
...
> **[ ESPACIO PARA IMAGEN: Captura de pantalla del juego en perspectiva top-down, mostrando una mazmorra con tonos morados, donde el personaje principal interactúa en un espacio con fantasmas, vasijas y puertas. Hay corazones de vida en la esquina superior izquierda. ]**
> 
> 
