# Capítulo 10: Física de Cuerpos Rígidos
...
| Estático | No, nunca
 ...
 | No se mueve nunca, sin importar qué la golpee

 | El suelo (una caja estática, ancha como todo el nivel)

 |
| Dinámico | Sí, por completo (gravedad, fuerzas, impulsos, colisiones)

 | Simulado íntegramente por el motor de física, cuadro a cuadro

 | El ave, y cada bloque de piedra, madera o alien de la torre

 |
| Cinemático | No, la gravedad y las fuerzas no lo afectan

 | Solo según una velocidad asignada a mano, pero empuja a los cuerpos dinámicos que se le apoyan

 | Ninguno, ya que Throw a Bird no lo usa, aunque World lo expone igual
...
```python
self.ground_body = world.create_static_body(
    GROUND_CENTER_X,
    center_y,
...
1. convertir las medidas de la forma de pixeles a metros, dividiendo cada una por PIXELS_PER_METER
2. calcular area_real = area de la forma, ya en metros, segun su formula geometrica (base x altura para una caja, pi * radio^2 para un circulo)
...
> **[ESPACIO PARA IMAGEN: Diagrama de secuencia UML mostrando el flujo de llamadas a `fixed_update()`. Inicia en `:Game` y pasa por `:ThrowABird`, `:PlayState`, hasta llegar a `:World`]**
> 
> 
> *Figura 10.2: Diagrama de secuencia de un paso fijo de simulación en Throw a Bird: Game.exec() llama a su propio _update(dt), que, tras la lógica del acumulador de tiempo del Capítulo 2, omitida aquí, invoca fixed_update() sobre sí mismo; ThrowABird.fixed_update() reenvía la llamada al estado actual (PlayState.fixed_update()), que a su vez llama a World.fixed_update(), el único punto de la cadena que efectivamente avanza el solver de pymunk.*
> 
...
> **[ESPACIO PARA IMAGEN: Captura de pantalla del gameplay del juego Throw a Bird mostrando un pájaro rojo, bloques de piedra, madera y aliens que conforman una torre cayéndose a un lado]**
> 
> 
...
> **[ESPACIO PARA IMAGEN: Diagrama de estado que muestra la máquina de estados finitos (FSM) con los estados "play" y "victory", y las flechas de transición cíclica de uno al otro]**
> 
> 
...
1. Calcular la densidad necesaria para que una caja de $100 \times 50$ píxeles tenga una masa objetivo de 300, dado un factor de conversión de 30 píxeles por metro, y verificarlo instanciando el cuerpo con `gale.physics` y leyendo su masa real.
